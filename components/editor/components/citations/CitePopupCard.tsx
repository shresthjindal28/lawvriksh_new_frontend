...
import type { ReferenceWithDocuments } from '@/types/reference-manager-api';
...
export default function CitePopupCard({
  ...
  const mapApiItemToRecommendation = (item: any): CombinedRecommendation => {
    ...
    const date =
      ...
      metadata.accessDate ||
      docMetadata.Date || // Capitalized in screenshot
      undefined;
    ...
    const summary =
      ...
      docMetadata.summary ||
      firstDoc.summary || // Explicit field in screenshot
      '';
    ...
    const source =
      ...
      metadata.genre ||
      docMetadata.source ||
      refData.ref_type || // Use ref_type as fallback source
      'Library';
  ...
  // Update position on window resize to prevent overflow
  useEffect(() => {
    if (!isVisible || showAllLibrary) return;

    const handleResize = () => {
      // Trigger a re-render by updating a state if needed
      // The position will be updated by the hook's scroll listener
      if (popupRef.current) {
        ...
        // Check if popup overflows viewport
        if (rect.right > viewportWidth - padding || rect.left < padding) {
          // Position will be recalculated by the hook
        }
  ...
  const getTabStyle = (tab: 'all' | 'web' | 'library'): React.CSSProperties => {
    ...
    return {
      ...
      height: '40px',
      fontSize: '16px',
      fontFamily: 'var(--lv-font-body, "Instrument Sans", Arial, sans-serif)',
      fontWeight: isActive ? 500 : 400,
      borderRadius: '8px',
      border: isActive ? 'none' : '1px solid var(--lv-border-input, #D7D8D3)',
      backgroundColor: isActive
        ? 'var(--lv-bg-button-dark, #12271D)'
        : 'var(--lv-bg-white, #FFFFFF)',
      color: isActive ? '#FFFFFF' : 'var(--lv-text-muted, #656565)',
      cursor: 'pointer',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'start' }}>
                    <button
                      ...
                      style={{
                        ...
                        fontSize: '13px',
                        fontWeight: 600,
                        color: 'var(--lv-accent-gold, #B48612)',
                        background: 'none',
                        border: 'none',
                      ...
                      onMouseEnter={(e) => {
                        if (!isAdding) {
                          e.currentTarget.style.backgroundColor = 'var(--lv-bg-toolbar, #F2EFE9)';
                        }
                      }}
                      onMouseLeave={(e) => {
      ...
      >
        <div
          ...
          style={{
            ...
            borderRadius: '16px',
            boxShadow: '0 4px 10.1px rgba(0,0,0,0.15)',
            display: 'flex',
            flexDirection: 'column',
            overflow: 'hidden',
            fontFamily: 'var(--lv-font-body, "Instrument Sans", Arial, sans-serif)',
        ...
        >
          ...
          >
            {(showAllLibrary ? isLoadingLibrary : isFetching) &&
            (!recommendations || showAllLibrary) ? (
              <div
                style={{
                  display: 'flex',
                  flexDirection: 'column',
              ...
              >
                ...
                <span style={{ color: '#6F7A8F', fontSize: '14px' }}>
                  {showAllLibrary
                    ? 'Loading your library references...'
              ...
              >
                {!showAllLibrary && !selectedText && !recommendations ? (
                  <>
                    <Scale
                      size={48}
                  ...
                  <p>
                    {showAllLibrary
                      ...
                      : 'No matching citations found in this source.'}
                  </p>
                )}
              </div>
            ) : (
                ...
                {recommendationsList?.map((rec, index) => (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'start' }}>
                      ...
                      rec.source === 'Library' ||
                      (recommendations?.personal_only &&
                        recommendations.personal_only.some(
                          (p) => p.title === rec.title || (p.link && p.link === rec.link)
                        )) ? (
                        <div
                          style={{
                            display: 'flex',
                            ...
                            gap: '6px',
                            fontSize: '14px',
                            color: '#6B7280', // Muted color for "Existing" status
                            fontWeight: 500,
                            marginLeft: '12px',
                          }}
                          title={
                            rec.source === 'Library'
                              ? 'This reference is already in your library'
                              : 'Added to library'
                          }
                        >
                          {rec.source === 'Library' ? (
                        ...
                        <button
                          ...
                          onMouseEnter={(e) => {
                            if (!isAdding && !isAddingToLibrary) {
                              ...
                              e.currentTarget.style.borderColor = 'var(--lv-border-input, #D7D8D3)';
                            }
                          }}
                          onMouseLeave={(e) => {
                            e.currentTarget.style.backgroundColor = 'transparent';
